...
def parse_args():
    ...
    parser.add_argument("--autoscaling-group-id", required=True, help="Auto Scaling Group ID")
...
def test_launch_template_exists(region_name, launch_template_id):
    try:
        ...
        if e.response["Error"]["Code"] == "InvalidLaunchTemplateId.NotFound":
            raise TestFailed(f"Launch template with ID {launch_template_id} does not exist")
        else:
            raise TestFailed(f"An error occurred: {e}")

    assert len(response["LaunchTemplates"]) == 1, f"Launch template with ID {launch_template_id} does not exist"
...
def test_autoscaling_group_exists(region_name, autoscaling_group_id):
    ...
    assert len(response["AutoScalingGroups"]) == 1, f"Auto Scaling Group with ID {autoscaling_group_id} does not exist"
...
def test_url_status_code(url, retries=10, delay=5):
    for attempt in range(retries):
        ...
        if attempt == retries - 1:
            raise TestFailed(f"Failed to make request to http://{url}:80 after {retries} retries")
